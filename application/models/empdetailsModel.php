 ...
 public function emp_leave_add()
    ...
    public function emp_permission_list()
    {
      if ($_SESSION['role'] == 'admin') {
        return $this->db->select('*')->from('emp_permission_details')->get()->result_array();
      } else if($_SESSION['department'] == 'MANAGEMENT'){
        return $this->db->select('*')->from('emp_permission_details')->where('manager_id',$_SESSION['emp_id'])->get()->result_array();
      }else{
        return $this->db->select('*')->from('emp_permission_details')->where('emp_id', $_SESSION['emp_id'])->get()->result_array();
      }
    }
    ...
    public function get_permission_count()
    {
       if ($_SESSION['role'] == 'admin') {
          ...
          $leave_count = $this->db->select('*')->from('emp_leave_details')->where('manager_id', $logged_manager_id)->where('leave_status=', 'Pending')->count_all_results();
       }else{
          $permission_count = 0;
          $leave_count = 0;
       }
    ...
    public function get_emp_leave_list()
    {
      if ($_SESSION['role'] == 'admin') {
        ...
        return $this->db->select('*')->from('emp_leave_details')->where('manager_id',$_SESSION['emp_id'])->get()->result_array();
      }else{
