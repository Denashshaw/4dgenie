...
class Client_model extends CI_Model
{
  ...
  public function insert_client_target_details()
  {
    $agents_list = $this->db->select('emp_id, agent_name')->from('emp_separation_managers')->where('manager_id', $_SESSION['emp_id'])->get()->result_array();
  ...
  {
    if ($_SESSION['department'] == 'MANAGEMENT') {
      $this->db->where('department.department', $_SESSION['sub_department']);
    } else {
      $this->db->where('department.department', $_SESSION['department']);
    }
    return $this->db->select('client_target.agent_id, client_target.agent_name,client_target.created_by,client_target.updated_by , client_target.id')->from('client_target')
      ...
      // ->where('department.department', $_SESSION['sub_department'])
      ->group_by('client_target.agent_id')
      ->get()->result();
  }

  public function get_task_setup_client_table()
  {
    if ($_SESSION['department'] == 'MANAGEMENT') {
      $this->db->where('department.department', $_SESSION['sub_department']);
    } else {
    ...
    return $this->db->select('client.client, client.id,client_target.target_value,client_target.created_by,client_target.updated_by')->from('client_target')
      ->join('client_task', 'client_task.id = client_target.task', 'left')
      ->join('client_sub_task', 'client_sub_task.id = client_target.sub_task', 'left')
      ->join('client', 'client.id = client_target.client', 'left')
      ->join('department', 'department.id = client_task.department', 'left')
      // ->where('department.department', $_SESSION['sub_department'])
      ->group_by('client_target.client')
      ->get()->result();
  }
  ...
  public function get_view_client_target_data()
  {
    if ($_SESSION['department'] == 'MANAGEMENT') {
      $this->db->where('department.department', $_SESSION['sub_department']);
    } else {
    ...
    //return $this->db->select('*')->from('client_target')->where('agent_id', $_POST['agent_id'])->get()->result();
    return $this->db->select('client_target.task as task_id, client_target.sub_task as sub_task_id, client_target.client as client_id, client.client, client_target.target_value, client_task.task, client_sub_task.sub_task, client_target.id as id')->from('client_target')
      ->join('client_task', 'client_task.id = client_target.task', 'left')
      ->join('client_sub_task', 'client_sub_task.id = client_target.sub_task', 'left')
      ->join('client', 'client.id = client_target.client', 'left')
      ->join('department', 'department.id = client_task.department', 'left')
  ...
  public function target_form_client_data()
  {
    // print_r($_POST);
    // die;
    $ans = [];
    $i = 0;
    foreach ($_POST as $param_name => $param_val) {
      $ans = explode('_', $param_name);
      $this->db->where('client', $ans[0]);
      $this->db->where('task', $ans[1]);
      $this->db->where('sub_task', $ans[2]);
      $this->db->update('client_target', array('target_value' => $param_val));
      $i++;
    }
